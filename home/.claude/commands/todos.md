...
Spawn one sub agent (use the Agent tool) per item, all at once, so they work in parallel and the main context stays clean. Issue all the Agent calls in a single message so they run concurrently. Spawn each sub agent with `isolation: "worktree"` so it starts inside its own git worktree and cannot work outside it. Each item gets its own worktree, so the sub agents do not interfere with each other. Give each sub agent its item and these instructions to follow:
